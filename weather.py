...
url_formated = URL.format(config['OPENWEATHERMAP']['lat'], config['OPENWEATHERMAP']['lon'], config['OPENWEATHERMAP']['units'], config['OPENWEATHERMAP']['appid'])
...
if isinstance(weather_icon, list):
    weather_icon = weather_icon[0] if current['sunrise'] < current['dt'] < current['sunset'] else weather_icon[1]
